class Tiles {
  ...
  createTilesOSMB(position){
    ...
    // OSMB Tiles are only available on zoomlevel 15. Therefore we need to calculate new tiles separately.
    ...
    for (f = 0; f < calculateTileDistance; f++) {
      for (h = 0; h < calculateTileDistance; h++) {
        ...
        tilenumber1 = parseInt(String(2)+String(curTileOSMB[0]+g)+String(curTileOSMB[1]+k));

        // add this new datatile only if it doesnt exist already in the map
        if(!this.pool.has(tilenumber1)){
          this.add(tilenumber1, new DataTile(tilenumber1,this.origin,[curTileOSMB[0]+g, curTileOSMB[1]+k])); //datatile
        }
        k++;
      }
      k = tileIncrement * -1;
